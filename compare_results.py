...
for _ in range(number_of_simulations):
    # same math as file 1, but no plotting
    num_base_stations = np.random.poisson(base_station_density * np.pi * (simulation_radius ** 2))
    if num_base_stations == 0: continue # if 0 towers spawn, skip this iteration
    
    distances_from_origin = simulation_radius * np.sqrt(np.random.uniform(0, 1, num_base_stations)) # source: LeetCode 478 Python
    distances_from_origin = distances_from_origin[distances_from_origin >= 1.0] # 1m exclusion
    if len(distances_from_origin) == 0: continue # if all bs were inside the 1m exclusion zone, skip this iteration
